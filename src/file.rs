...
impl FileReference {
    ...
    where   R: AsyncRead + Unpin,
        ...
        let r: Arc<ReedSolomon<galois_8::Field>> = Arc::new(ReedSolomon::new(data, parity).unwrap());
        let mut done = false;
        ...
        while !done {
            ...
            if wrote_data {
                ...
                parts_fut.push(tokio::spawn(async move {
                    r.encode(&mut bufs)?;
                    let buf_futures: FuturesOrdered<_> = bufs.drain(..)
                        .map(|data| tokio::spawn(async move {
                            let hash = Sha256Hash::from_buf(&data);
                            (data, hash)
                        }))
                        .collect();
                    let (mut bufs, mut hashes): (Vec<Vec<u8>>, Vec<Sha256Hash>) = buf_futures
                        .map(|res| res.unwrap())
                        .unzip().await;
                    let mut writers = destination.get_writers(&hashes).await?;
                    let mut write_results = bufs.drain(..)
                        .zip(writers.drain(..))
                        .zip(hashes.drain(..))
                        .map(|((data, (mut writer, location)), hash)| async move {
                            writer.write_shard(&data).await
                                .map(|_| HashWithLocation{
                                    sha256: hash,
                                    locations: location,
                                })
                        })
                    ...
                    if write_results.iter().any(Result::is_err) {
                        Err(write_results.drain(..).filter_map(Result::err).next().unwrap().into())
                    } else {
                        let mut hashes_with_location:
                            Vec<HashWithLocation<Sha256Hash>> = write_results.drain(..)
    ...
    where   W: AsyncWrite + Unpin {
        ...
        for FilePart{data,parity,..} in &self.parts {
            ...
            let r: ReedSolomon<galois_8::Field> = ReedSolomon::new(data.len(), parity.len()).unwrap();
            let mut shard_bufs = data.iter()
                .chain(parity.iter())
            ...
            for mut buf in shard_bufs.drain(..).take(data.len()).filter_map(|op| op) {
...
impl From<Location> for LocationDestination {
    fn from(loc: Location) -> Self {
        ...
        match loc {
            Location::Local(path) =>
                LocationDestination::Local(LocalFiles::new(&path)),
            Location::Http(url) =>
                LocationDestination::Http(HttpFiles::new(url)),
        }
    }
}
...
impl CollectionDestination for Vec<WeightedLocation> {
    ...
    >(
        ...
        let mut res = vec![];
        for addr in addrs {
            let mut total_weight: usize = working_locations.iter()
                .map(|WeightedLocation{weight, ..}| *weight)
                .sum();
            if total_weight == 0 {
                working_locations = self.iter().collect();
                total_weight = working_locations.iter()
                    .map(|WeightedLocation{weight, ..}| *weight)
                    .sum();
            }
            ...
            for (index, WeightedLocation{weight, location}) in working_locations.iter().enumerate() {
            ...
            if let Some(index) = found_index {
                res.push(
                    working_locations.remove(index)
                        .location.get_writers(&[format!("{}",addr)]).await?
                        .drain(..)
                        .next()
                        .unwrap()
                )
